...
use malachite::num::arithmetic::traits::{Parity, RoundToMultipleOfPowerOf2};
    ...
    for y in cutoff..source_img.height() {
        for x in 0..source_img.width() {
            ...
            if target_x < w && target_y < h {
                ...
                target_pixel.0[0] = u8::rounding_from(
                    &(Rational::from(target_pixel.0[0]) * &scale),
                    RoundingMode::Nearest,
                )
                .0;
                target_pixel.0[1] = u8::rounding_from(
                    &(Rational::from(target_pixel.0[1]) * &scale),
                    RoundingMode::Nearest,
                )
                .0;
                target_pixel.0[2] = u8::rounding_from(
                    &(Rational::from(target_pixel.0[2]) * scale),
                    RoundingMode::Nearest,
                )
                .0;
...
fn get_background_color(dir_path: &str, track_name: &str) -> Rgba<u8> {
    let background_color_path = format!("{dir_path}/{track_name}/background-color.txt");
    let file = File::open(background_color_path).unwrap();
...
fn add_score_image(
    ...
    score_img = score_img.resize(
        u32::rounding_from(&target_score_width_px, RoundingMode::Nearest).0,
        u32::rounding_from(&target_score_height_px, RoundingMode::Nearest).0,
    ...
    if right {
        copy_from_with_trans_2(
            canvas,
            &score_img,
            i32::rounding_from(&((page_width + inner_margin) * DPI), RoundingMode::Nearest).0,
            i32::rounding_from(&score_vertical_margin_px, RoundingMode::Nearest).0,
            0,
            false,
        );
    } else {
        copy_from_with_trans_2(
            canvas,
            &score_img,
            i32::rounding_from(&(outer_margin * DPI), RoundingMode::Nearest).0,
            i32::rounding_from(&score_vertical_margin_px, RoundingMode::Nearest).0,
            0,
            false,
        );
    ...
    match page {
        BookPage {
            ...
            *previous_track_name = Some(track_name.clone());
            ...
            img = img.resize(
                u32::rounding_from(&target_screenshot_width_px, RoundingMode::Nearest).0,
                u32::rounding_from(&target_screenshot_height_px, RoundingMode::Nearest).0,
                FilterType::Nearest,
            );
            let mut canvas = DynamicImage::new_rgb8(
                u32::rounding_from(&((&page_width << 1) * DPI), RoundingMode::Nearest).0,
                u32::rounding_from(&(&page_height * DPI), RoundingMode::Nearest).0,
            );
            ...
            draw_rectangle(
                ...
                0,
                0,
                u32::rounding_from(&((&page_width << 1) * DPI), RoundingMode::Nearest).0 - 1,
                u32::rounding_from(&(&page_height * DPI), RoundingMode::Nearest).0 - 1,
            );
            ...
            sprite_img = sprite_img.resize(
                SPRITE_WIDTH_PX,
                u32::rounding_from(&target_sprite_height_px, RoundingMode::Nearest).0,
                FilterType::Nearest,
            );
            let gap = 1000;
            for i in 0..10 {
            ...
            if *page_number == 0 {
                canvas
                    .copy_from(
                        &img,
                        u32::rounding_from(&(&outer_margin * DPI), RoundingMode::Nearest).0,
                        u32::rounding_from(&screenshot_vertical_margin_px, RoundingMode::Nearest).0,
                    )
                    .unwrap();
            }
            ...
            let page_number_index = *i
                - ((contents_count - 1)
                    .round_to_multiple_of_power_of_2(1, RoundingMode::Ceiling)
                    .0
                    + 1);
            if !first {
            ...
            *i += 1;
            ...
            copy_from_with_trans_2(
                &mut canvas,
                &page_number_img,
                i32::rounding_from(&(&page_width * DPI), RoundingMode::Nearest).0 + 504,
                272,
                PAGE_NUMBER_CUTOFF_PX,
                true,
            );
            ...
            *i += 1;
            canvas
                .view(
                    0,
                    0,
                    u32::rounding_from(&(&page_width * DPI), RoundingMode::Nearest).0,
                    u32::rounding_from(&(&page_height * DPI), RoundingMode::Nearest).0,
                )
                .to_image()
                .save(&out_path_1)
...
pub fn generate_book(dir_path: &str) {
    ...
    command
        ...
        .arg(&format!(
            "{}",
            u32::rounding_from(
                &(page_width * Rational::const_from_unsigned(72)),
                RoundingMode::Nearest
            )
            .0
        ))
