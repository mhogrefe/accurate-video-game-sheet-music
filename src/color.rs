...
use palette::Lab;
use palette::{Component, Pixel, Srgb};

pub(crate) fn raw_rgb_to_srgb(raw_rgb: Rgba<u8>) -> Srgb {
    Srgb::from_raw_slice_mut(&mut [
        f32::from(raw_rgb.0[0]),
        f32::from(raw_rgb.0[1]),
        f32::from(raw_rgb.0[2]),
    ])[0]
}
...
#[inline]
pub fn from_f64<T: FromF64>(c: f64) -> T {
    T::from_f64(c)
}
